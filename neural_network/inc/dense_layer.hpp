 ...
 *
 * @param RELU Enumerator for selecting ReLU (Rectified Linear Unit)-
 * @param TANH Enumerator for selecting Tanh (the hyperbolic tangent function).
 ********************************************************************************/
enum class ActFunc { RELU, TANH };

/********************************************************************************
 * @brief Class for implementation of dense layers in neural networks.
 ********************************************************************************/
class DenseLayer {
  public:
    ...
    DenseLayer(const std::size_t num_nodes, 
               ...
               const enum ActFunc act_func = ActFunc::RELU);
     ...
     ********************************************************************************/
    std::size_t numNodes(void) const;
  ...
  public:
     ...
     * 
     * @param input Reference to vector holding the new input values.
     ********************************************************************************/
     ...
     ********************************************************************************/
    void optimize(const std::vector<double>& input, const double learning_rate = 0.01);

    /********************************************************************************
     * @brief Changes the number of nodes and weights per node in the dense layer.
     * 
     * @note  All node parameters are reinitialized after the resize operation.
     * 
     * @param num_nodes   The new number of nodes in the dense layer.
     * @param num_weights The new number of weights in the dense layer.
     ********************************************************************************/
    void resize(const std::size_t num_nodes, const std::size_t num_weights_per_node);
  ...
  private:
    ...
    enum ActFunc act_func_{ActFunc::RELU};       
};
